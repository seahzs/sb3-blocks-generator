...
let viewer = {};
const HAT_BLOCKS = ['event_whenflagclicked', 'event_whenkeypressed', 'event_whengreaterthan', 'event_whenthisspriteclicked', 'event_whenstageclicked',
                    'event_whenbackdropswitchesto', 'event_whenbroadcastreceived', 'control_start_as_clone', 'procedures_definition',
                    'boost_whenColor', 'boost_whenTilted', 'ev3_whenButtonPressed', 'ev3_whenDistanceLessThan', 'ev3_whenBrightnessLessThan',
                    'gdxfor_whenGesture', 'gdxfor_whenForcePushedOrPulled', 'gdxfor_whenTilted', 'makeymakey_whenMakeyKeyPressed', 'makeymakey_whenCodePressed',
                    'microbit_whenButtonPressed', 'microbit_whenGesture', 'microbit_whenTilted', 'microbit_whenPinConnected', 'wedo2_whenDistance', 'wedo2_whenTilted'];
document.getElementById('sb3Upload').addEventListener('change', (e) => {
    ...
    parseSb3File(sb3File).then((projectJson)=>{loadProject(JSON.parse(projectJson))});
});
spriteList.addEventListener('change', generateScratchblocks);
spriteList.addEventListener('click', generateScratchblocks);
blockSize.addEventListener('change', () => {renderSvg().then(renderPNG())});
new ClipboardJS('#copyText');
document.getElementById('downloadText').addEventListener('click', generateTxtFileDownload);
document.getElementById('downloadSvg').addEventListener('click', generateSvgFileDownload);
document.getElementById('generatePng').addEventListener('click', renderPNG);
document.getElementById('downloadPng').addEventListener('click', generatePngFileDownload);
...
async function loadProject(projJSON){
    ...
    Array.from(spriteList.children)
        ...
        .forEach(elem => {spriteList.removeChild(elem)});
    projectData.targets.filter(t => !t.isStage)
        .forEach(target => {
...
function hideContainers(toHide){textContainer.hidden = blocksContainer.hidden = toHide; errorMsg.hidden = !toHide}
function generateScratchblocks(){
    if (!projectData || Object.keys(projectData).length === 0) {
        ...
        if (!target) {target = projectData.find(t => t.isStage);};
        if (Object.keys(target.blocks).length==0){
            ...
            scratchblocksCode = hatBlocks.map(hatKey => parseSB3Blocks.toScratchblocks(hatKey, target.blocks, 'en', {tab: ' '.repeat(4), variableStyle: 'as-needed',})).join('\n\n');
...
async function renderSvg(){
    const renderOpts = {style: 'scratch3', languages: ['en'], scale: blockSize.value};
    viewer = scratchblocks.newView(scratchblocks.parse(scratchblocksCode, renderOpts), renderOpts);
    await viewer.render();
    const svgStr = viewer.exportSVGString();
    svgImg.src = "data:image/svg+xml;utf8," + svgStr.replace(/[#]/g, encodeURIComponent);
};
...
function generateTxtFileDownload(){
    const fileName = sb3File.name.slice(0,-4)+'_'+spriteList.value.replace('_','')+'.txt';
...
function generateSvgFileDownload(){
    const fileName = sb3File.name.slice(0,-4)+'_'+spriteList.value.replace('_','')+'.svg';
    triggerDownload(svgImg.src, fileName, 'image/svg+xml');
};
function generatePngFileDownload(){
    const fileName = sb3File.name.slice(0,-4)+'_'+spriteList.value.replace('_','')+'.png';
    renderPNG().then(imgURL => {triggerDownload(imgURL, fileName, 'image/png')});
};
